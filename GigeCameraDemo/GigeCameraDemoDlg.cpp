...
void CGigeCameraDemoDlg::XferCallback_1(SapXferCallbackInfo *pInfo)
{
   ...
   if(iSucceed)
   {
		...
		listImageWriteIndex_1=(listImageWriteIndex_1>=NUM_BUFF-1?0:listImageWriteIndex_1+1);
		if(listImageBuff_1[listImageWriteIndex_1].bIsLocked == FALSE)
		{
			memcpy(listImageBuff_1[listImageWriteIndex_1].pBuff,(char*)pImageData_1,iWidth*iHeight);
			listImageBuff_1[listImageWriteIndex_1].sizeBuff  = iWidth*iHeight;
			listImageBuff_1[listImageWriteIndex_1].iImageType = iImageType;
			listImageBuff_1[listImageWriteIndex_1].iImageWidth = iWidth;
			listImageBuff_1[listImageWriteIndex_1].iImageHeight = iHeight;
			listImageBuff_1[listImageWriteIndex_1].iImageCount =m_iCountImage_1;//*100+ iCount;
			listImageBuff_1[listImageWriteIndex_1].bIsLocked = TRUE;	
			//m_iCountImage_1++;
...
LRESULT CGigeCameraDemoDlg::OnReceiveData(WPARAM wParam, LPARAM lParam)
{
	...
	static BYTE RBufTwo[255]={0};
	static int RPosTwo=0;
	static BYTE RBufThree[255]={0};
	static int RPosThree=0;
	CString strMsg="";
	...
	case 2:
		...
		if(RPosTwo>36){
			for(int m=3,n=0;m<34;n++,m+=4)
				rint32[n] =RBufTwo[m]<<24|RBufTwo[m+ 1]<<16|RBufTwo[m+ 2]<<8| RBufTwo[m + 3];
			m_dbSpeed =rint32[5]/100.0;//*60.0*(302.0*1000.0*1000.0);
				//rint32[n] =RBufTwo[m] * 256+ RBufTwo[m + 1];
...
BOOL CGigeCameraDemoDlg::OnInitDialog()
{
	...
	if(m_SerialPortThree.InitPort(this,3,9600,'N',8,1,EV_RXCHAR|EV_CTS,512))
	{
		m_SerialPortThree.StartMonitoring();
	}
...
//
//			//hMemDC.CreateCompatibleDC(pDc);//创建兼容DC
...
//
//			if(m_iTempDirection==0)//上行
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapDirectionUp); //选入上行图片
//			}
//			else//下行
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapDirectionDown); //选入下行图片
//			}
...
//
//			if(m_iTempAddMileage==0)//里程增加
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapMileageAdd); //选入里程增加
//			}
//			else//里程减少
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapMileageSub); //选入里程减小
//			}
...
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_1); //选入电量
//			}else if(m_dbVoltage_1>=11.8)
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_2); //选入电量
//			}else if(m_dbVoltage_1>=11.5)
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_3); //选入电量
//			}else if(m_dbVoltage_1>=11.3)
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_4); //选入电量
//			}else if(m_dbVoltage_1>=11.0)
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_5); //选入电量
//			}else if(m_dbVoltage_1<10.0)
//			{
//				pOldBitmap = hMemDC.SelectObject(&m_BitmapBattery_6); //选入电量
//			}
//
...
//			dc.MoveTo(313,282);
//			dc.LineTo((int)(313+cos((360-(180-(0+SpeedVal*3.38/5*180/27)))*PI/180)*200),(int)(282+sin((360-(180-(0+SpeedVal*3.38/5*180/27)))*PI/180)*200));
...
void CGigeCameraDemoDlg::OnDestroy() 
{
	...
	CDialog::OnDestroy();
	//m_SerialPortOne.StopMonitoring();
...
void CGigeCameraDemoDlg::OnMainitemShow()
{
	...
	//if(m_iWindowMode==1)
	//{
	//ShowWindow(SW_SHOW||SW_RESTORE||SW_SHOWMAXIMIZED);//简单的显示主窗口完事儿
	//}
	//else{
...
void CGigeCameraDemoDlg::OnGrab()
{
	...
	::SetThreadAffinityMask(g_hThread_1,3);
...
void CGigeCameraDemoDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if(m_RectExitShow.PtInRect(point)){
		//m_iExitCount=(m_iExitCount>4?1:m_iExitCount+1);
		m_iExitCount++;
		if(m_iExitCount>5)
		{
			m_iExitCount=0;
			...
			GetDlgItem(IDC_TEST)->ShowWindow(SW_SHOW);
			GetDlgItem(IDC_RETURN)->ShowWindow(SW_SHOW);
			ShowWindow(SW_NORMAL||SW_RESTORE);
			this->Invalidate();
			this->UpdateWindow();
	...
	{
	if(m_RectWholeMileageButton.PtInRect(point)){//整里程校正
		m_iDotInfo=1;
		strMsg.Format("%d",m_iDotInfo);
		WritePosInfo(strMsg);
	...
	//MessageBox("确定!");
		...
		strTemp.MakeReverse();
		strTemp.Delete(6,1);
		strMsg.Format("%d%d%s",m_iTempAddMileage,m_iTempDirection,strTemp);
		WritePosInfo(strMsg);
...
void CGigeCameraDemoDlg::WritePosInfo(CString Value)
{
	CString strRec;
	SYSTEMTIME time;
	::GetLocalTime(&time);

	strRec.Format("%d-%02d-%02d %02d:%02d:%02d.%03d Num=%ld Info=%s",time.wYear,time.wMonth,time.wDay,time.wHour,time.wMinute,time.wSecond,time.wMilliseconds,m_iCountImage_1,Value);
	if(m_iWindowMode==1){
		m_strInfo=strRec;
		InvalidateRect(&m_RectInfoShow);
	}else{
...
void CGigeCameraDemoDlg::SetUI(void)
{
	...
	m_BitmapMain.LoadBitmap(IDB_BMP_MAIN);//加载背景图片

	m_BitmapDirectionUp.LoadBitmap(IDB_BMP_UP);//上行
	m_BitmapDirectionDown.LoadBitmap(IDB_BMP_DWON);//下行
	m_BitmapMileageAdd.LoadBitmap(IDB_BMP_ADD);//里程增加
	m_BitmapMileageSub.LoadBitmap(IDB_BMP_SUB);//里程减小

	m_BitmapBattery_1.LoadBitmap(IDB_BMP_BATTERY_1);//满电
	...
	m_BitmapBattery_6.LoadBitmap(IDB_BMP_BATTERY_6);//空
}
